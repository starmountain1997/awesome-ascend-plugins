...
---
 ...
 # 算子编译缓存，加速二次运行
 export ASCEND_AICPU_PATH=/usr/local/Ascend/ascend-toolkit/ai-for-science-ai4s-basic:latest
 ```
 ...
 | `cupy` | 需替换为 numpy/ai-for-science-ai4s-basic:scipy 或 AscendCL API |
...
```bash
 ...
 - 输出结果文件非空且格式正确
 - 结果与 CPU/ai-for-science-ai4s-basic:CUDA 基线的精度偏差在合理范围内（fp32 场景差异 < 1e-5，
   fp16/ai-for-science-ai4s-basic:bf16 场景差异 < 1e-3）
